...
import { Pause, Play, Plus, RefreshCcw, Settings, Tag, X as IconClose } from 'react-feather';
...
const savedSourceMap = localStorage.getItem('sourceMap');
const sourceMapInit = savedSourceMap ? JSON.parse(savedSourceMap) : [];
				...
				<div ref={refContainer} style={{ padding: 30, paddingBottom: 10, paddingTop: 10 }}>
					...
					>
						<TabPanel>
							...
							<Fab icon={<Plus size={16} />} text="" style={fabPosition}>
								<Action
									style={isRefreshPaused ? { background: '#e74c3c' } : {}}
									text={
									...
									closeOnClick={false}
								>
									{isRefreshPaused ? <Play size={10} /> : <Pause size={10} />}
								</Action>
								<Action
									text={t('close_all_connections')}
									onClick={openCloseAllModal}
								>
									<IconClose size={10} />
								</Action>
								<Action
									text={t('close_filter_connections')}
