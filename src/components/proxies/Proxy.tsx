...
function getLabelColor({ number }: { number?: number } = {}, httpsTest: boolean) {
	const delayMap = {
...
function getProxyDotBackgroundColor(latency: { number?: number }, httpsTest: boolean) {
...
const ProxySmallImpl = memo(function ProxySmallImpl({
	now,
	name,
	proxy,
	latency,
...
const ProxyNameTooltip = memo(function ProxyNameTooltip({
	children,
	label,
...
const TfoIcon = memo(function TfoIcon() {
	return (
		<svg
...
const ProxyImpl = memo(function ProxyImpl({
	now,
	name,
	proxy,
	latency,
	...
	const udpText = useMemo(() => {
		if (!proxy.udp) return '';
		return proxy.xudp ? 'XUDP' : 'UDP';
	}, [proxy.udp, proxy.xudp]);

	const proxyTypeText = useMemo(() => formatProxyType(proxy.type), [proxy.type]);
	...
	const nameSpan = useMemo(() => <span>{name}</span>, [name]);
		...
		>
			<div className={cx(s0.proxyName, s0.row)}>
				...
				<span
					...
					style={{ paddingLeft: 4, opacity: 0.6, color: '#51A8DD' }}
				>
					{udpText}
				</span>
			</div>
